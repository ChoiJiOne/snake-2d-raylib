...
void MainPhase::Enter()
{
    ...
    Vector2 scorePosition = Vector2{
        static_cast<float>(config->GetWindowWidth()) / 3.0f,
        static_cast<float>(config->GetWindowHeight()) / 20.0f
    };
    ...
    Vector2 levelPosition = Vector2{
        2.0f * static_cast<float>(config->GetWindowWidth()) / 3.0f,
        static_cast<float>(config->GetWindowHeight()) / 20.0f
    };
    ...
    Vector2 boardPosition = Vector2{ 
        ...
        static_cast<float>(config->GetWindowHeight()) / 2.0f
    };
    Board* board = ActorManager::Get().Create<Board>(
        boardPosition, 
        config->GetTileSize(), 
        config->GetRowTileCount(), 
        config->GetColTileCount()
    );
    ...
    Snake* snake = ActorManager::Get().Create<Snake>(
        ...
        score,
        level,
        static_cast<EDirection>(config->GetStartDirection())
    );
